...
public interface UserRepository extends JpaRepository<User, Integer> {
			...
			@Query(value = "select count(email) from trangtintuc.user  where email = :mail"
					,nativeQuery = true)
			int checkMail (@Param("mail") String mail);
			
			@Query(value = "select id from trangtintuc.user where tai_khoan = :ten",nativeQuery = true)
			int getID(@Param("ten") String ten);
			
			@Modifying
			@Transactional
			@Query(value = "insert into trangtintuc.user_role (user_id,role_id) values (:idUser,3)",nativeQuery = true)
